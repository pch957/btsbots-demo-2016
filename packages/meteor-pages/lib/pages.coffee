  ...
  setInfiniteTrigger: ->
    ...
    @scrollBox.scroll _.bind (
      _.throttle ->
        ...
        if (window.innerHeight*(2-@infiniteTrigger) + @scrollBox.scrollTop()  >= l)
          #console.log "hello: #{window.innerHeight} #{@scrollBox.scrollTop()} #{@infiniteStep} #{l}"
          @sess("limit", @sess("limit") + @infiniteStep)
          
          if @sess("limit") + 25 > @lastPage*@perPage 
            #console.log "i want page #{@lastPage}, limit: #{@sess('limit')}"
